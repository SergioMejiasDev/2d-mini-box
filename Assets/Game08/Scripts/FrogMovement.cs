...
public class FrogMovement : MonoBehaviour
{
    ...
    /// <summary>
    /// El objeto móvil sobre el que la rana se va a subir.
    /// </summary>
    [Header("Water Area")]
    GameObject parent;
    /// <summary>
    ...
    /// <summary>
    /// Función que permite que la rana se mueva en función de los inputs introducidos.
    /// </summary>
    void CheckInputs()
    {
        if (Time.timeScale != 1 || isDie)
        {
            return;
        }

        if (Input.GetKeyDown(KeyCode.W) && !RayMovement(Vector2.up))
        {
            jumpSound.Play();
            ResetValues();
        ...
        else if (Input.GetKeyDown(KeyCode.D) && !RayMovement(Vector2.right))
        {
            jumpSound.Play();
            ResetValues();
        ...
        else if (Input.GetKeyDown(KeyCode.S) && !RayMovement(-Vector2.up))
        {
            jumpSound.Play();
            ResetValues();
            destination = (Vector2)transform.position - 0.5f * Vector2.up;
        ...
        else if (Input.GetKeyDown(KeyCode.A) && !RayMovement(-Vector2.right))
        {
            jumpSound.Play();
            ResetValues();
            destination = (Vector2)transform.position - 0.45f * Vector2.right;
    ...
    {
        // Si la rana salta sobre un objeto en movimiento en el agua, se desplazará sobre él.
